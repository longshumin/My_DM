...
#load files
train = pd.read_csv('C:\\train.csv', parse_dates=['timestamp'])
test = pd.read_csv('C:\\test.csv', parse_dates=['timestamp'])
id_test = test.id
...
num_boost_rounds = 395  # This was the CV output, as earlier version shows
model = xgb.train(dict(xgb_params, silent=0), dtrain, num_boost_round= num_boost_rounds)
...
df_train = pd.read_csv("C:\\train.csv", parse_dates=['timestamp'])
df_test = pd.read_csv("C:\\test.csv", parse_dates=['timestamp'])
df_macro = pd.read_csv("C:\\macro.csv", parse_dates=['timestamp'])
...
num_boost_rounds = 430  # From Bruno's original CV, I think
model = xgb.train(dict(xgb_params, silent=0), dtrain, num_boost_round=num_boost_rounds)
